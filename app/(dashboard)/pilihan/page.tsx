...
export default function PilihanPage() {
  ...
  useEffect(() => {
    const fetchPilihans = async () => {
      ...
      if (data) {
        ...
        setPilihans(dummyPilihanHariIni)
      }
      setIsMounted(true)
    }
    fetchPilihans()
  ...
  if (!isMounted) {
    return <LoadingSpinner text="Memuat pilihan..." />
  }

  const filteredPilihans = pilihans.filter((item) => {
      ...
      <div className="@container/main flex flex-1 flex-col gap-2 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="flex flex-col gap-4 py-4 md:gap-4 md:py-6 px-4 lg:px-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <h1 className="text-2xl md:text-2xl font-bold tracking-tight">Pilihan Hari Ini</h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            ...
            {filteredPilihans.length === 0 && (
              <div className="col-span-full py-12 text-center text-muted-foreground">
                Tidak ada pilihan yang cocok dengan pencarian Anda.
              </div>
            )}
          </div>
